...
import com.example.abhinav.myecho.adapters.FavoriteAdapter
...
class FavoriteFragment : Fragment() {
    ...
    var getListFromTheDatabase: ArrayList<Songs>? = null
    ...
    fun display_favorites_by_searching() {
        if (favoriteContent?.checkSize() as Int > 0) {
            ...
            getListFromTheDatabase = favoriteContent?.queryDBList()
            var fetchListfromDevice = getSongsFromPhone()
            if (fetchListfromDevice != null) {
                for (i in 0..fetchListfromDevice?.size - 1) {
                    for (j in 0..getListFromTheDatabase?.size as Int - 1) {
                        if (getListFromTheDatabase?.get(j)?.songID === fetchListfromDevice?.get(i)?.songID) {
                            refreshList?.add((getListFromTheDatabase as ArrayList<Songs>)[j])
                        }
                    }
                }
            }
